...
taxonomy_info_2 = pd.merge(left=final_table, right=Species, on='genome_id', how='inner')

taxonomy_info_2 = taxonomy_info_2.drop_duplicates()
...
table = pd.pivot_table(taxonomy_info_2, values=['GeneHit'], index=['genome_id'],
                    columns=['Gene'], fill_value=0)

# merge table and phenotype and Genus:
taxa_pheno = taxonomy_info_2.drop('Gene', 1)
taxa_pheno = taxa_pheno.drop('GeneHit', 1)

taxa_pheno = taxa_pheno.drop_duplicates() 

table = table.drop_duplicates()

taxa_pheno =taxa_pheno.set_index('genome_id')
#merge phenotype data with eggnog data
taxonomy_info_2 = table.join(taxa_pheno)

#Make a arff file for each genus:            
def write_to_arff_genus(antibiotic, genus, taxonomy_info_2):
    antibiotic_df = taxonomy_info_2[taxonomy_info_2['antibiotic'] == antibiotic]
    antibiotic_df = taxonomy_info_2[taxonomy_info_2['Genus'] == genus]
    ...
    antibiotic_df = antibiotic_df.drop('Genus', 1)
    ...
    antibiotic_df_array_sep = antibiotic_df_array_sep.replace(',,', ',')
    outfile_genus = f"{antibiotic}_{genus}_taxa_files_TEST_TEST.arff"
...
for antibiotic in antibiotics:
    for genus in genuses:
        write_to_arff_genus(antibiotic, genus, taxonomy_info_2)
        
# Make a arff file that is all but one genus:

def write_to_arff_except_genus(antibiotic, genus, taxonomy_info_2):
    antibiotic_df = taxonomy_info_2[taxonomy_info_2['antibiotic'] == antibiotic]
    antibiotic_df = taxonomy_info_2[taxonomy_info_2['Genus'] != genus]
    antibiotic_df = antibiotic_df.reset_index()
    antibiotic_df = antibiotic_df.drop_duplicates()
    ...
    antibiotic_df_array_sep = antibiotic_df_array_sep.replace(',,,', ',')
    antibiotic_df_array_sep = antibiotic_df_array_sep.replace(',,', ',')
    outfile_genus = f"{antibiotic}_{genus}_not_taxa_files_TEST_TEST.arff"
    att_out_genus = open(outfile_genus, "w")
    i = 'Gene'
    att_out_genus.write('@RELATION    ' + i + '\n')
    for x in Genes:
...
for antibiotic in antibiotics:
    for genus in excluded_genuses:
        write_to_arff_except_genus(antibiotic, genus, taxonomy_info_2)
