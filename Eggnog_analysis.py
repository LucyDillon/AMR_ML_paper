...
table = pd.pivot_table(grouped, values=['Family_freq'], index=['genome_id'], columns=['Gene_families'], fill_value=0)
# MIC analysis:
...
def write_to_arff(antibiotic, outfile_antibiotic, final_table):
    ...
    for x in Gene_families:
        att_out_antibiotic.write('@ATTRIBUTE Gene_families' + x + '  REAL\n')
    name = 'phenotype'
...
for antibiotic in antibiotics:
    outfile_antibiotic = f"{antibiotic}_EGGNOG_test.arff"
